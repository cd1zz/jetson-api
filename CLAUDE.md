...
OpenAI-compatible API for local LLM inference on NVIDIA Jetson AGX Orin. Provides a unified FastAPI interface that routes requests to multiple llama.cpp backend servers running different models:
- **Text Models**: DeepSeek R1 7B, Qwen 2.5 7B
- **Vision Model**: Qwen3-VL-8B (multimodal image+text, computer use capabilities)
...
curl -X POST http://localhost:9000/v1/chat/completions \
  -H "Authorization: Bearer your-api-key-here" \
  -H "Content-Type: application/json" \
  -d '{"model": "qwen3-vl-8b", "messages": [{"role": "user", "content": [{"type": "text", "text": "Describe this image"}, {"type": "image_url", "image_url": {"url": "data:image/png;base64,..."}}]}]}'
...
- Image upload for vision models (auto-shows for Qwen3-VL)
...
- `get_gpu_stats()` - GPU utilization and frequency via Jetson sysfs (`/sys/devices/platform/bus@0/17000000.gpu/`)
- `get_cpu_stats()` - CPU utilization from /proc/stat (two-sample delta for instantaneous reading)
- `get_memory_stats()` - RAM usage from /proc/meminfo (unified memory shared between CPU and GPU)
...
- Vision models (Qwen3-VL) use the llama.cpp `/v1/chat/completions` endpoint directly with structured messages
...
- `call_llama_server_vision()` / `stream_llama_server_vision()` - Vision model handlers (Qwen3-VL)
- `call_llama_server_embeddings()` - Embeddings generation via `/v1/embeddings` endpoint
- `check_backend_health()` - Backend health checks using `/health` or `/props` endpoint
...
- `deepseek_base_url` / `qwen_base_url` / `qwen3_vl_base_url` / `qwen3_embedding_base_url` - Backend URLs
...
│ DeepSeek R1     │ Qwen 2.5        │ Qwen3-VL 8B     │ Qwen3 Embed 8B  │
│ (text)          │ (text)          │ (vision)        │ (embeddings)    │
│ :8081           │ :8082           │ :8084           │ :8085           │
└─────────────────┴─────────────────┴─────────────────┴─────────────────┘
```

Each llama-server instance runs a single quantized GGUF model with CUDA acceleration. The vision model (Qwen3-VL) includes a multimodal projection (mmproj) file for image understanding. The embeddings model runs in embeddings mode (`--embeddings` flag) for vector generation. The API routes requests based on the `model` field and endpoint.

## Configuration

Environment variables (`.env`):
- **Required**: `API_KEY` - Must be set to a secure value for production
- **Backend URLs**: `DEEPSEEK_BASE_URL`, `QWEN_BASE_URL`, `QWEN3_VL_BASE_URL`, `QWEN3_EMBEDDING_BASE_URL` - Point to llama-server instances
...
- `llama-qwen3-vl.service` - Qwen3-VL-8B vision backend (port 8084)
...
- `jetson-clocks.service` - Locks GPU/CPU clocks to max on boot

All services run as `User=user` with binaries and models under `/home/user/`.

### Installation

```bash
# Enable and start all services (starts on boot)
sudo systemctl enable llama-deepseek llama-qwen llama-qwen3-vl llama-qwen3-embedding jetson-api
sudo systemctl start llama-deepseek llama-qwen llama-qwen3-vl llama-qwen3-embedding jetson-api

# Check status
sudo systemctl status jetson-api
sudo systemctl status llama-qwen3-vl

# View logs
sudo journalctl -u jetson-api -f
sudo journalctl -u llama-qwen3-vl -f
```
...
- **Timeouts**: HTTP clients use 300s timeout for inference requests, 5s for health checks.
- **Unified Memory**: Jetson uses unified memory (CPU and GPU share the same 64 GB pool). System RAM stats reflect total available memory for all workloads.
